...
#Create Hive Table for Storm topology
...
# Build from source
echo "*********************************Building Vaccine Manufacturing Monitor Storm Topology"
cd $ROOT_PATH/VaccineManufacturingMonitor
mvn clean package
...
/usr/hdp/current/kafka-broker/bin/kafka-topics.sh --create --zookeeper $AMBARI_HOST:2181 --replication-factor 1 --partitions 1 --topic IncomingBioReactorEvents
/usr/hdp/current/kafka-broker/bin/kafka-topics.sh --create --zookeeper $AMBARI_HOST:2181 --replication-factor 1 --partitions 1 --topic IncomingFiltrationEvents
